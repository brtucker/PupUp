...
//  PupUp
//
//  Created by Blake Tucker on 2/14/16.
...
let apiKey = "f1be3179bcbdc89e2abe2326bded58d2"
...
class Flickr {
    ...
    func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void){
        ...
        NSURLSession.sharedSession().dataTaskWithRequest(searchRequest, completionHandler: {
            ...
            do {
                resultsDictionary = try NSJSONSerialization.JSONObjectWithData(data!, options: .MutableContainers) as! NSDictionary
            }
            catch {
                JSONError = NSError(domain: "", code: 9999, userInfo: nil)
